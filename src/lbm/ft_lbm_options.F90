...
#include "lbm_definitions.h"
  ...
  contains
    ! Mohsin updated this
    function OptionsCreate(comm, n_comb) result(options)
    ...
    subroutine OptionsSetUp(options)
      ...
      ! Mohsin added this
      options%kwrite = options%kwrite / options%ncombs
      ...
      str_length = 6
      if (StringCompareIgnoreCase(name, test_discretization, str_length)) then
      ...
      if (options%flow_disc == NULL_DISCRETIZATION) then
         ...
         ! Mohsin updated this
         code_val = 1
         call LBMError(PETSC_COMM_SELF, code_val, 'Invalid Discretization', ierr)
      end if
