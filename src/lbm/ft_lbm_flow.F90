...
contains
      ...
      do lcv=2,flow%ncomponents
        ...
        if (flow%components(lcv)%viscosity < -990.) then
          ...
          if (abs(flow%components(lcv)%relax%tau - 1.) < eps) then
             ...
             code_val = 1
             call LBMError(flow%comm, code_val, &
  ...
  subroutine FlowParseBC(flow, options, boundary, bname)
    ...
    ! Mohsin updated the following (since -bc_periodic_* is not working for petsc)
    !bcvalue = PETSC_FALSE
    if (flow%grid%info%nproc_x > 1) then
       bcvalue = PETSC_TRUE
    else
       bcvalue = PETSC_FALSE
    end if
    ...
    if (bcvalue) then
      ! Mohsin updated the followings
      code_val = 1
      if (done) call LBMError(flow%comm, code_val, &
    ...
    if (bcvalue) then
      ...
      if (flow%ncomponents.eq.1) then
        ...
        if (.not.flag) then
          ...
          ! Mohsin updated the followings
          code_val = 1
          call LBMError(flow%comm, code_val,"Momentum BC on boundary "//trim(bname)// &
        ...
        do m=1,flow%ncomponents
          ...
          if (.not.flag) then
            ...
            ! Mohsin updated the followings
            code_val = 1
            call LBMError(flow%comm, code_val, "Momentum BC on boundary "// &
    ...
    if (bcvalue) then
      ...
      if (.not.flag) then
        ...
        ! Mohsin updated the followings
        code_val = 1
        call LBMError(flow%comm, code_val, "Velocity BC on boundary "//trim(bname)// &
             " was provided the wrong number of components.", ierr)
  ...
  subroutine FlowUpdateBCPressureOutlet(flow, walls)
    ...
    if ((DABS(flow%components(1)%gf(1)) > eps) .or. &
      ...
      ! Mohsin updated the followings
      code_val = 1
      call LBMError(PETSC_COMM_SELF, code_val, &
  ...
  subroutine FlowUpdateDensityFromPressure(flow, pressure, rho1frac, rho)
    ...
    if (flow%ncomponents.eq.1) then
      ...
      ! Mohsin updated the followings
      code_val = 1
      call LBMError(flow%comm, code_val, "Invalid number of components for flow", ierr)
