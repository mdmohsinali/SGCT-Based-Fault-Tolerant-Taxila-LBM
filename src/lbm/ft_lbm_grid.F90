...
contains
  ! Mohsin updated this
  function GridCreate(comm, proc_x, proc_y, proc_z, comp_g, rep_comb, n, nx, ny, nz) result(grid)
    MPI_Comm comm
    ! Mohsin added this
    ...
    ! Mohsin updated this
    grid%info => InfoCreate(comm, proc_x, proc_y, proc_z, comp_g, rep_comb, n, nx, ny, nz)
    grid%nda = 0
    grid%name = ''
  ...
  subroutine GridSetPhysicalScales(grid, ierr)
    ...
    if (abs(grid%info%gridsize(Y_DIRECTION)-grid%length_scale) > eps) then
       ...
       code_val = 1
       call LBMError(PETSC_COMM_WORLD, code_val, &
    ...
    if (grid%info%ndims > 2) then
       if (abs(grid%info%gridsize(Z_DIRECTION)-grid%length_scale) > eps) then
          ! Mohsin updated the followings
          code_val = 1
          call LBMError(PETSC_COMM_WORLD, code_val,'Lattice sizes must be equal, check grid parameters.', ierr)
