...
contains
  ...
  subroutine EOSSetFromOptions(eos, options, ierr)
    ...
    str_length = 3
    if (StringCompareIgnoreCase(test_eos_name, eos_name, str_length)) then
    ...
    if (.not.done) then
      ! Mohsin updated the followings
      code_val = 1
      call LBMError(PETSC_COMM_WORLD, code_val, 'Invalid EOS type', ierr)
  ...
  subroutine EOSApply_PR(eos, rho, psi, g_mm, m, dist)
    ...
    do i=1,dist%info%rgxyzl
      ...
      if (tmp < 0) then
        ...
        ! Mohsin updated the followings
        code_val = 1
        call LBMError(PETSC_COMM_WORLD, code_val, 'PR EOS inner sqrt went negative', ierr)
