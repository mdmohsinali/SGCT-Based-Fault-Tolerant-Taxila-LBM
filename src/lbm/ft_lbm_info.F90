...
#include "lbm_definitions.h"
  ...
  type, public:: info_type
     MPI_Comm comm
     ! Mohsin added ndoubles
     PetscInt ndims, ndoubles

     PetscInt NX,NY,NZ
     PetscBool,pointer,dimension(:) :: periodic
     ! Mohsin added this
     LOGICAL (KIND = 1) :: is_comp_grid, is_rep_comb
...
contains
  ! Mohsin updated this
  function InfoCreate(comm, proc_x, proc_y, proc_z, comp_g, rep_comb, n, nx, ny, nz) result(info)
    MPI_Comm comm
    ! Mohsin added this
    PetscInt proc_x, proc_y, proc_z, n, nx, ny, nz
    ...
    info%nproc_z = proc_z
    ! Mohsin updated this
    info%is_comp_grid = comp_g
    info%is_rep_comb = rep_comb
    info%ndoubles = n
  ...
  subroutine InfoSetFromOptions(info, options, ierr)
    ...
    ! -- grid size
    ! Mohsin disabled the followings
