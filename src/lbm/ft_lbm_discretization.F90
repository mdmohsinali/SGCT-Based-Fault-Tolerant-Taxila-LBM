...
contains
  ...
  subroutine DiscretizationSetDerivOrder(disc, isotropy_order)
    ...
    if ((isotropy_order.eq.0).or.(isotropy_order.eq.4).or. &
       ...
       code_val = 1
       call LBMError(PETSC_COMM_SELF, code_val, 'Invalid derivative order', ierr)      
  ...
  subroutine DiscretizationSetUp(disc)
    ...
    case DEFAULT
       ! Mohsin updated the followings
       code_val = 1
       if (disc%ndims < 0 ) call LBMError(PETSC_COMM_SELF, code_val, 'Invalid Discretization', ierr)
